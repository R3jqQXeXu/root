  ...
  FixTemplateSphere(lmp, narg, arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"spheres") == 0) {
      ...
      {
          if (narg < iarg + 4*nspheres) error->all("Illegal fix particletemplate/multisphere command, not enough arguments");
    ...
    else error->all("Fix particletemplate/multisphere: need to define spheres for the template");
  }
...
void FixTemplateMultiplespheres::calc_center_of_mass()
{
  ...
  for(int i = 0; i < ntry; i++)
  {
      ...
      for(int j = 0; j < 3; j++)
        x_try[j] = x_min[j]+(x_max[j]-x_min[j])*random->uniform();

      alreadyChecked = false;
      for(int j = 0; j < nspheres; j++)
      {
          dist_j = sqrt(pow(x_try[0]-x_sphere[j][0],2.) + pow(x_try[1]-x_sphere[j][1],2.) + pow(x_try[2]-x_sphere[j][2],2.));
          // only count once if contained in multiple spheres
          if (alreadyChecked) break;
          if(dist_j < r_sphere[j])
          {
